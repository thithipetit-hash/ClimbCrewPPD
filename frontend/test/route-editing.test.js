...
test("le graphique CPR reste dans la largeur mobile et la suppression est compacte", async () => {
  ...
  assert.match(source, /variant="remove"/);
  assert.match(source, /className="progression-realisation-remove"/);
  assert.match(source, /aria-label="Supprimer cette réalisation"/);

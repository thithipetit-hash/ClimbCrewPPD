...
test("le backend cible les comptes actifs et conserve un accusé de lecture individuel", async () => {
  const [serverSource, migrationSource, broadcastSource] = await Promise.all([
    readFile(new URL("../../backend/server.js", import.meta.url), "utf8"),
    readFile(new URL("../../backend/database/migrations/001_baseline.sql", import.meta.url), "utf8"),
    readFile(new URL("../../backend/broadcast-message-routes.js", import.meta.url), "utf8"),
  ]);
  assert.match(migrationSource, /create table if not exists broadcast_messages/);
  assert.match(migrationSource, /create table if not exists broadcast_message_recipients/);
